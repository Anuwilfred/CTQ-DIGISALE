...
const CACHE_NAME = 'ctorq-shell-v28';
const APP_SHELL = [
  './',
  './index.html',
  './manifest.json',
...
self.addEventListener('fetch', (event) => {
  ...
  if (req.url.endsWith('/data/companies.json') || req.url.endsWith('/data/rss_signals.json') || req.url.endsWith('/data/active_projects.json') || req.url.endsWith('/data/changelog.json')) {
    event.respondWith(
      fetch(req).then((res) => {
        if (res.ok) {
